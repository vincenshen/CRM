...
class Paginator(object):
    def __init__(self, current_page, per_page_number, all_data_count, base_url, condition_str, order_str, page_range=5):
        """
    ...
    def page_str(self):
        ...
        if self.current_page <= 1:
            ...
            prev = '<li><a href="%s?_p=%s%s%s">上一页</a></li>' % (self.base_url, self.current_page - 1, self.condition_str, self.order_str)
        ...
        for i in range(start, end):
            if self.current_page == i:
                page_number = '<li class="active"><a href="%s?_p=%s%s%s">%s</a></li>' % (self.base_url, i, self.condition_str, self.order_str, i)
            else:
                page_number = '<li><a href="%s?_p=%s%s%s">%s</a></li>' % (self.base_url, i, self.condition_str, self.order_str, i)
        ...
        if self.current_page >= self.all_page:
            ...
            nex = '<li><a href="%s?_p=%s%s%s">下一页</a></li>' % (self.base_url, self.current_page + 1, self.condition_str, self.order_str)
